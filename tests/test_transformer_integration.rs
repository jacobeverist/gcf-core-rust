...
fn test_scalar_vs_discrete_comparison() {
    ...
    assert!(scalar.get_output().borrow().state.num_set() > 0);
    assert!(discrete.get_output().borrow().state.num_set() > 0);

    // Patterns will differ (scalar has overlapping, discrete doesn't)
    assert_ne!(scalar.get_output().borrow().state, discrete.get_output().borrow().state);
}
...
fn test_multiple_transformers_pipeline() {
    ...
    assert_eq!(mode.get_output().borrow().state.num_set(), 256); // 1024/4
    assert_eq!(stability.get_output().borrow().state.num_set(), 128);
}
...
fn test_scalar_semantic_properties() {
    ...
    for i in 0..(transformers.len() - 1) {
        let overlap = transformers[i]
            .get_output().borrow().state
            .num_similar(&transformers[i + 1].get_output().borrow().state);
    ...
    let overlap_0_1 = transformers[0]
        .get_output().borrow().state
        .num_similar(&transformers[5].get_output().borrow().state);
...
fn test_discrete_categorical_independence() {
    ...
    for i in 0..num_categories {
        for j in (i + 1)..num_categories {
            let overlap = transformers[i]
                .get_output().borrow().state
                .num_similar(&transformers[j].get_output().borrow().state);
...
fn test_persistence_temporal_tracking() {
    ...
    for i in 0..10 {
        ...
        if i % 3 == 0 {
            patterns.push(pt.get_output().borrow().state.clone());
        }
    }
...
fn test_mixed_transformer_types() {
    ...
    assert_eq!(temperature.get_output().borrow().state.num_set(), 128);
    assert_eq!(weather_type.get_output().borrow().state.num_set(), 204); // 1024/5
    assert_eq!(temp_stability.get_output().borrow().state.num_set(), 128);
    ...
    assert_eq!(weather_type.get_output().borrow().state.num_set(), 204);
...
fn test_transformer_state_independence() {
    ...
    assert_ne!(t1.get_output().borrow().state, t2.get_output().borrow().state);

    // Change t1 shouldn't affect t2
    t1.set_value(0.8);
    t1.execute(false).unwrap();

    let t2_before = t2.get_output().borrow().state.clone();
    t2.execute(false).unwrap();
    let t2_after = t2.get_output().borrow().state.clone();
...
fn test_clear_all_transformers() {
    ...
    assert_eq!(scalar.get_output().borrow().state.num_set(), 0);
    assert_eq!(discrete.get_output().borrow().state.num_set(), 0);
    assert_eq!(persistence.get_output().borrow().state.num_set(), 0);
...
fn test_time_series_encoding() {
    ...
    for &value in time_series.iter() {
        ...
        assert_eq!(scalar.get_output().borrow().state.num_set(), 128);
    }

    // Can access history
    let output = scalar.get_output();
...
fn test_categorical_time_series() {
    ...
    for &action in actions.iter() {
        ...
        assert_eq!(discrete.get_output().borrow().state.num_set(), 204); // 1024/5
    }
}
...
fn test_deterministic_encoding() {
    ...
    // Should be identical
    assert_eq!(s1.get_output().borrow().state, s2.get_output().borrow().state);
    assert_eq!(d1.get_output().borrow().state, d2.get_output().borrow().state);
    assert_eq!(p1.get_output().borrow().state, p2.get_output().borrow().state);
}
...
fn test_boundary_value_analysis() {
    ...
    discrete.execute(false).unwrap();
    assert_eq!(discrete.get_output().borrow().state.num_set(), 102);
...
fn test_complete_workflow() {
    ...
    assert_eq!(temp_sensor.get_output().borrow().state.num_set(), 256);
    assert_eq!(location.get_output().borrow().state.num_set(), 512); // 2048/4
    assert_eq!(stability.get_output().borrow().state.num_set(), 256);

    // Can access history
    let output = temp_sensor.get_output();
    let output_borrow = output.borrow();
